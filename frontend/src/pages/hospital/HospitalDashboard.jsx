  ...
  useEffect(() => {
    const fetchHospitalData = async () => {
      try {
        ...
        const profileRes = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/facility/profile`, {
        ...
        // Fetch blood stock
        const stockRes = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/hospital/blood/stock`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        // Fetch blood requests
        const requestsRes = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/hospital/blood/requests`, {
          headers: { Authorization: `Bearer ${token}` },
        });
