...
const BloodLabDonor = () => {
  ...
  const searchDonors = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/blood-lab/donors/search?term=${term}`,
  ...
  const loadRecentDonations = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/blood-lab/donations/recent`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const markDonation = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/blood-lab/donors/donate/${selectedDonor._id}`,
        donationData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const quickDonation = async (donorId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/blood-lab/donors/donate/${donorId}`,
