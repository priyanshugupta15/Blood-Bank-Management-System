...
const LabManageRequests = () => {
  ...
  const loadRequests = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/blood-lab/blood/requests`, {
  ...
  const updateStatus = async (id, action) => {
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/blood-lab/blood/requests/${id}`,
        { action },
        { headers: { Authorization: `Bearer ${token}` } }
      );
